...
- **Dependency & build caching** — caches `deps/` and `_build/` with keys segmented by OS, `MIX_ENV`, Erlang version, Elixir version, `mise.toml`, and `mix.lock`
...
2. **Restore deps & build cache** — restores `deps/` and `_build/` using a composite key of `runner.os`, `mix-env`, Erlang version, Elixir version, `mise.toml` hash, and `mix.lock` hash. Falls back to partial key matches when `mix.lock` has changed, then to the previous non-versioned key format for compatibility.
